...
* Features
  ...
  * one configuration file `config/license_finder.yml`
  (deprecation of `doc/dependency_decisions.yml`)
  * configuration file structure overhaul
  * deprecation of command line subcommands `approvals`, `approvals`,
  `dependencies`, `ignored_dependencies`, `ignored_groups`, `licenses`,
  `project_name`, `whitelist`. Edit directly the config file
